...
namespace IdentityServer3.EntityFramework
{
    public static class DbModelBuilderExtensions
    {
        ...
        public static void RegisterUserChildTablesForDelete<TUser>(this DbContext ctx)
            where TUser : User
        {
            ctx.Set<TUser>().Local.CollectionChanged +=
                delegate (object sender, NotifyCollectionChangedEventArgs e)
                {
                    if (e.Action == NotifyCollectionChangedAction.Add)
                    {
        ...
        public static void ConfigureUsers(this DbModelBuilder modelBuilder, string schema)
        {
            modelBuilder.Entity<User>().ToTable(EfConstants.TableNames.User, schema);

            modelBuilder.Entity<User>()
                .HasMany(u => u.Claims).WithRequired(x => x.User).WillCascadeOnDelete();

            modelBuilder.Entity<UserClaim>().ToTable(EfConstants.TableNames.UserClaim, schema);
        }
    }
}