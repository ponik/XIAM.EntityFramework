...
namespace IdentityServer3.EntityFramework.Entities
{
    public static class EntitiesMap
    {
        ...
        static EntitiesMap()
        {
            ...
            {
                config.CreateMap<Entities.Scope, Core.Models.Scope>(MemberList.Destination)
                ...
                config.CreateMap<Entities.ScopeClaim, Core.Models.ScopeClaim>(MemberList.Destination);
                config.CreateMap<Entities.ScopeSecret, Core.Models.Secret>(MemberList.Destination)
                    .ForMember(dest => dest.Type, opt => opt.Condition(srs => srs != null));

                config.CreateMap<Entities.ClientSecret, Core.Models.Secret>(MemberList.Destination)
                     .ForMember(dest => dest.Type, opt => opt.Condition(srs => srs != null));
                config.CreateMap<Entities.Client, Core.Models.Client>(MemberList.Destination)
                ...
                config.CreateMap<Entities.User, Core.Services.InMemory.InMemoryUser>(MemberList.Destination)
                    .ForMember(x => x.Claims, opt => opt.MapFrom(src => src.Claims.Select(x => new Claim(x.Type, x.Value))));
            }).CreateMapper();
        }

        public static Core.Models.Scope ToModel(this Entities.Scope s)
        {
            if (s == null) return null;
            return Mapper.Map<Entities.Scope, Core.Models.Scope>(s);
        }

        public static Core.Models.Client ToModel(this Entities.Client s)
        {
            if (s == null) return null;
            return Mapper.Map<Entities.Client, Core.Models.Client>(s);
        }

        public static Core.Services.InMemory.InMemoryUser ToModel(this Entities.User s)
        {
            if (s == null) return null;
            return Mapper.Map<Entities.User, Core.Services.InMemory.InMemoryUser>(s);
        }
    }
}
